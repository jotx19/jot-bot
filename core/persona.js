/**
 * Bot identity — override with BOT_NAME and BOT_PERSONA in .env
 */

export function getBotName() {
  return (process.env.BOT_NAME || 'tinyjot').trim();
}

/**
 * Core identity block prepended to every user-facing reply (via DEFAULT_SYSTEM_PROMPT).
 */
export function getIdentityPrompt() {
  const name = getBotName();
  const extra = process.env.BOT_PERSONA?.trim();
...
- Be direct, warm, and helpful — like a capable personal aide who remembers context over time.`;
  ...
  if (extra) {
    block += `\n\nAbout the user and how to behave:\n${extra}`;
  }
